	...
	const onStartTest = async () => {
		...
		if (!specificTestId) {
			const specificTestSession =
				await testSessionService.startSpecificTestSession(
					Number(sessionId),
					{ test_type: "AUDITORY" }
				);
			...
			<div className="text-gray-700 text-left space-y-4 max-w-lg">
				...
				<div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded">
					<p className="text-sm text-yellow-800">
						<strong>Lưu ý:</strong> Hãy đeo tai nghe hoặc bật âm
						thanh để nghe rõ các câu hỏi. Test sẽ tự động phát âm
						thanh khi bắt đầu mỗi câu hỏi.
					</p>
				</div>
			</div>
