    ...
    return (
        <div className="tree-and-progress">
            <div className="tree-container">
                ...
                <div
                    className="trunk"
                    style={{ '--trunk-height': `${trunkHeight}px` } as React.CSSProperties}
                />
                <div
                    className="foliage foliage-main"
                    style={{ '--scale': mainFoliageScale, '--trunk-height': `${trunkHeight}px` } as React.CSSProperties}
                >
                ...
                <div
                    ...
                    style={{ '--scale': leftBranchScale, '--trunk-height': `${trunkHeight}px` } as React.CSSProperties}
                >
                ...
                <div
                    ...
                    style={{ '--scale': rightBranchScale, '--trunk-height': `${trunkHeight}px` } as React.CSSProperties}
                >
...
const GameCanvas: React.FC = () => {
    ...
    const currentWords = useMemo(() => getRandomItems(allWords, wordsPerGame), [gameId]);
    ...
    const currentWord = useMemo(() => currentWords[currentWordIndex], [currentWords, currentWordIndex]);

    // --- NEW: Helper to play System Sounds (Correct/Wrong) ---
    const playSystemSound = (type: 'correct' | 'wrong') => {
        try {
            const soundPath = `/minigame3/audio/${type}.mp3`;
            const audio = new Audio(soundPath);
            audio.volume = 0.8; // Slightly softer than the main word
    ...
    // --- UPDATED: Handle Letter Click with Correct/Wrong Sounds ---
    const handleLetterClick = (letter: string) => {
        ...
        if (firstEmptyBlankIndex !== -1) {
            ...
            if (letter === expectedLetter) {
                // --- CORRECT CASE ---
                playSystemSound('correct'); // Play Correct Sound 🎵
                ...
                if (isWordFinished) {
                    // Delay word pronunciation slightly so it doesn't overlap the "ding" completely
                    // setTimeout(() => playWordSound(), 600);
                ...
                // --- INCORRECT CASE ---
                playSystemSound('wrong'); // Play Wrong Sound 🎵

                (newPuzzleDisplay[firstEmptyBlankIndex] as any) = { char: letter, status: "incorrect" };

                setShowFullWord(true);
                // Removed immediate playWordSound() here. 
                // Usually better to just hear the "Wrong" buzzer so the user knows they made a mistake.
    ...
    const progressPercentage = totalBlanksInGame > 0
        ? (totalCorrectBlanks / totalBlanksInGame) * 100
        : 0;

    const imagePath = `/minigame3/images/${currentWord?.img || 'placeholder.jpg'}`;
            ...
            <div className="flex flex-col items-center justify-between p-8 bg-gradient-to-br from-blue-100 via-blue-200 to-sky-200 text-slate-800 relative">
                ...
                <div className="flex flex-col sm:flex-row gap-4 mb-8 z-10">
                    ...
                    <button
                        ...
                        onClick={() => navigate('/test/minigame')}
                    >
                        Return to Center
                    </button>
                </div>
        ...
        <div className="game-wrapper">
            {showIntro && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md transition-all duration-500">
                    <h1 className="absolute top-10 text-4xl font-bold text-white drop-shadow-lg">Minigame 3: <span className="text-green-400">Cây Thần Kỳ🌳</span></h1>
                    <div className="bg-white p-8 rounded-3xl shadow-2xl max-w-lg w-full relative animate-[fadeInUp_0.5s_ease-out]">
                        ...
                        <div className="space-y-6 text-lg text-slate-700 mb-8 px-2">
                            <div className="flex items-start gap-4">
                                ...
                                <p className="leading-tight pt-0.5">Nhìn hình ảnh minh họa và lắng nghe âm thanh của từ.</p>
                            </div>
                            <div className="flex items-start gap-4">
                                ...
                                <p className="leading-tight pt-0.5">Chọn các chữ cái trên màn hình để điền vào chỗ trống cho đúng.</p>
                            </div>
                            <div className="flex items-start gap-4">
                                ...
                                <p className="leading-tight pt-0.5">Mỗi câu trả lời đúng sẽ giúp <span className="font-bold text-green-600">Cây Thần Kỳ🌳</span> của bạn lớn lên!</p>
                            </div>
                        </div>
            ...
            <button className="back-btn" onClick={() => navigate('/test/minigame')}>
                ❮ Back to Minigame Center
            </button>
            ...
            <div className="game-container">
                <div className="left-panel">
                    <div className={`image-container ${showFullWord ? 'show-full-word' : ''}`}>
                        <img
                            ...
                            onError={(e) => { (e.target as HTMLImageElement).src = 'https://placehold.co/350x280/e0e0e0/757575?text=Image+Missing'; }}
                        />
                ...
                <div className="right-panel">
                    <div className="progress-info">
                        <div className="word-counter">Word {currentWordIndex + 1} / {currentWords.length}</div>
                    </div>
