        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div className="relative rounded-2xl bg-rose-50 border border-rose-100 shadow-[0_20px_60px_-20px_rgba(244,63,94,0.18)] overflow-hidden min-h-[520px] md:min-h-[600px]">
            ...
            <div className="absolute left-1/2 -translate-x-1/2 bottom-4 w-[66%] md:w-[62%]">
              ...
              <div className="mt-1 text-center text-sm text-rose-600">
                {progress}%
              </div>
            </div>
          </div>
          ...
          <div className="rounded-2xl bg-white/95 border border-rose-100 shadow-[0_10px_40px_-15px_rgba(244,63,94,0.18)] p-6 flex flex-col">
            ...
            <div className="text-slate-800 text-sm space-y-3 mb-6">
              <div>
                <span className="text-rose-600 font-semibold">Mô tả:</span>{" "}
                {item?.description}
              </div>
              <div className="italic">
                <span className="text-rose-600 not-italic font-semibold">
                  Ví dụ:
                </span>{" "}
                "{item?.example}"
              </div>
              {item?.hint ? (
                <div className="text-rose-600/90">Gợi ý: {item.hint}</div>
              ) : null}
            </div>
            ...
            <form onSubmit={submit} className="mt-auto">
              <input
                ...
                value={st.input}
                onChange={(e) =>
                  setSt((p) => ({ ...p, input: e.target.value }))
                }
              ...
              <div className="mt-3 flex items-center justify-between">
                <div className="text-slate-500 text-sm">
                  Câu {st.index + 1}/{MG1_ITEMS.length}
                </div>
                <button
                  type="submit"
            ...
            {st.feedback !== "idle" && (
              <div
                className={`mt-3 text-sm ${
                  st.feedback === "right" ? "text-emerald-600" : "text-rose-600"
                }`}
              >
                {st.feedback === "right"
                  ? "Chính xác! Bộ phận robot đã được lắp."
                  : "Chưa đúng, thử lại ở câu tiếp theo nhé."}
              </div>
            )}
          </div>
        </div>
