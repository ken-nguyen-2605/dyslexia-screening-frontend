...
const QUESTIONS: Question[] = [
    ...
    { id: 2, sentence: "Hoàng Sa Trường Sa là của Việt Nam.", correctAnswer: true }, 
    ...
    const renderRobotImage = () => {
        ...
        return (
            <div className="relative w-80 h-80 rounded-lg overflow-hidden shadow-xl">
                ...
                <div 
                    className="absolute inset-0 w-full h-full bg-gradient-to-br from-pink-100 via-cyan-100 to-yellow-100 border-4 border-pink-300 flex flex-col items-center justify-center"
                >
                     {imageRevealPercent === 0 && (
                        <span className="text-xl text-pink-700 font-bold">Trả lời đúng để lắp ráp robot!</span>
                    )}
                </div>
                ...
                >
                    ...
                    {/* 3. Thanh laser */}
                    {imageRevealPercent < 100 && imageRevealPercent > 0 && feedbackVisible && isCorrect && (
                        <div 
                            className="absolute left-0 right-0 h-1 bg-red-500 laser-pulse"
                            style={{ top: `${imageRevealPercent}%`, transform: 'translateY(-50%)' }}
                        />
                    )}
                </div>
            </div>
    ...
    if (testFinished) {
        return (
            <div className="text-center p-10 bg-white rounded-xl shadow-2xl max-w-lg mx-auto">
                <h2 className="text-3xl font-bold text-pink-600 mb-4">Yeah! Bạn đã lắp ráp xong Robot 🤖</h2>
                <p className="text-xl text-gray-700">Final Score: <span className="font-extrabold text-green-500">{score} / {TOTAL_QUESTIONS}</span></p>
    ...
    return (
        <div className="flex flex-col items-center bg-white/90 border-4 border-pink-200 p-10 rounded-[2em] items-center space-y-7 shadow-xl max-w-lg w-full mx-auto">
            ...
            <div className="w-full h-2 bg-pink-100 rounded-full">
                <div className="bg-pink-400 h-2 rounded-full transition-all duration-700" style={{ width: `${imageRevealPercent}%` }} />
            </div>
            ...
            {feedbackVisible && (
                <div className={`text-2xl font-extrabold ${isCorrect ? 'text-green-600' : 'text-red-600'} transition-opacity duration-300`}>
